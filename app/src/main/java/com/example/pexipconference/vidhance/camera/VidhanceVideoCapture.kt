...
import com.vidhance.appsdk.utils.AppSdkConfiguration
...
class VidhanceVideoCapture(private val cameraIndex: Int, private val sensorModeIndex: Int, private var vidhanceInterface: VidhanceInterface) :
    VideoCapturer {
    ...
    override fun initialize(surfaceTextureHelper: SurfaceTextureHelper?, context: Context?, observer: CapturerObserver?) {
        ...
        this.surfaceTextureHelper!!.startListening(VideoSink { videoFrame: VideoFrame? ->
            observer!!.onFrameCaptured(VideoFrame(videoFrame!!.buffer, getRotationDegrees(context!!), videoFrame!!.timestampNs))
        })
    }
    ...
    private fun initCamera(inputResolution: Size) {
        ...
        simpleCamera!!.setOnStaticMetaListener(object : OnStaticMetaListener {
            override fun onStaticMetaUpdated(
                ...
                if (AppSdkConfiguration.USE_SEPARATE_OUTPUT_BUFFER) {
        ...
        vidhanceInterface!!.configureOutput(outputSurface)
